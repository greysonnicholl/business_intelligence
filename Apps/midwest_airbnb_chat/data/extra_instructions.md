...
-`host_is_superhost` and `instant_bookable` are text values t and f, not booleans. Use t for true and f for false when filtering these fields.
- When calculating an average of `review_scores_rating`, ignore rows where `review_scores_rating` is NULL so it doesn't effect the output.
